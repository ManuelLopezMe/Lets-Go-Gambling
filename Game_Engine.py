...
class PlayGame:
                ...
                if action == 'split' and len(player_hand)==2 and player_hand[0] == player_hand[1]:
                    ...
                    while right_value <= 21: 
                        ...
                        if right_action == 'double':
                            right = PlayHand(player_hand=right_hand, None, deck)
                    ...
                    while left_value <= 21:
                        ...
                        if left_action == 'double':
                            left = PlayHand(left_hand=right_hand, None, deck)
                    ...
                    while dealer_value < 21:
                        x, dealer_value=PlayHand(None, dealer_hand, deck).dealer_turn()
                        if x == 'done':
                            break
    ...
    def play_game(self, bankroll=100, num_rounds=10, num_decks = 2):
        ...
        while (rounds_played <= num_rounds) or bankroll > 0: #play until we've reach round limit or if player goes bankrupt
            ...
            if wager > self.bankroll: #wager can't be higher than the players bankroll
                print("Not enough to wager, enter amount less than ${bankroll}".format(bankroll))
                wager = int(input('select wager amount'))
            bankroll += self.play_round(wager, player_hand, dealer_hand, deck)
            rounds_played += 1
            if len(deck) <= round(total_deck_size*.2): #reshuffle after we've gone through 20% of the deck
                deck.shuffle()
